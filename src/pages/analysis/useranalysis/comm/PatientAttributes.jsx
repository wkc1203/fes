...
class Widget extends React.Component {
  ...
  getProvince=(pageNum=1,numPerPage=6)=>{
    ...
    dispatch({
      type: 'userAnalysis/getpatientprovincestatistics' ,
      payload:{
  ...
  getProvinceData=()=>{
    ...
    const { PatientProvince = {} } = modelData;
    const { recordList = [], currentPage = 1, numPerPage = 10, totalCount = 0 ,countResultMap={}} = PatientProvince;
    let list =[]
  ...
  getCity=(provinceCode='',pageNum=1,numPerPage=6)=>{
    const { dispatch } = this.props;
    dispatch({
      type: 'userAnalysis/getpatientcitystatistics' ,
      payload:{
          hisId:sessionStorage.getItem('hisId'),
          pageNum,
          numPerPage,
  ...
  getCityData=()=>{
    const { modelData = {} } = this.props;
    const { PatientCity = {} } = modelData;
    const { recordList = [], currentPage = 1, numPerPage = 10, totalCount = 0 ,countResultMap={}} = PatientCity;
    let list =[]
    if(recordList.length>0){
      list = recordList.map((item,key)=>{
        return {
  ...
  getSex=()=>{
    ...
    dispatch({
      type: 'userAnalysis/getpatientsexstatistics' ,
      payload:{
          hisId:sessionStorage.getItem('hisId'),
  ...
  getSexData=()=>{
    ...
    if(PatientSex.length>0){
      ...
      dv.source(PatientSex).transform({
        type: "percent",
        field: "totalSum",
        dimension: "sex",
        as: "percent"
  ...
  getAge=()=>{
    ...
    const {startDate='',endDate='',type=''} =this.state;
    dispatch({
      type: 'userAnalysis/getpatientagestatistics' ,
      payload:{
          hisId:sessionStorage.getItem('hisId'),
          startDate:startDate?startDate+' 00:00:00':'',
          endDate:endDate?endDate+' 23:59:59':'',
  ...
  getAgeData=()=>{
    const { modelData = {} } = this.props;
    const { PatientAge = {} } = modelData;
    let aval=[],tval=10;
    if(PatientAge.length>0){
      PatientAge.map((item)=>{

        // if(item.totalSum==0)item.totalSum= parseInt(Math.random()*1000) 
  ...
  render() {
    ...
    const { modelData = {} } = this.props;
    const { AllPatientProvince = {} } = modelData;

    // console.log(AllPatientProvince,data,'123')

    data.features.map((item)=>{
      // let totalSum=0;
      if(AllPatientProvince.recordList){

        AllPatientProvince.recordList.length>0?AllPatientProvince.recordList.map((item2)=>{
      ...
      <div className="">
        ...
        <div className="userAttributeBOX">
            ...
            <Select
              ...
              >
                <Option value="1">新增就诊人数</Option>
                <Option value="2">累计就诊人数</Option>
              </Select>
            ...
            <div className="AttributeChart">
                ...
                <div>
                <Chart placeholder height={400} data={getAgeData.PatientAge} scale={getAgeData.cols} padding={[30,10,40,60]}>
                  <Axis name="age" />
                  <Axis name="totalSum" 
