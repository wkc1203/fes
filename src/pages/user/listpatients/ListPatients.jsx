...
class Widget extends React.Component {
  ...
  query = (pageNum=1) => {
    ...
    dispatch({
      type: 'ListPatients/PatientsList',
      payload: {
  ...
  render() {
    const totalCount = this.props.PatientsList ? this.props.PatientsList.totalCount : 0;
    const currentPage = this.props.PatientsList ? this.props.PatientsList.currentPage : 0;
    const columns = [
      {
        title: '用户',
        dataIndex: 'realName',
    ...
    const data = this.props.PatientsList && this.props.PatientsList.recordList.length > 0 ?
    this.props.PatientsList.recordList.map((item, key) => {
      return {
        key: key + 1,
        realName: item.realName,
        sex: item.sex || '未知',
        name: item.name || '未知',
        patCardNo: item.patCardNo || '未知',
    ...
    return (
      <div className="page-order-list">
        <div className="query-box">
          <span>就诊人：</span>
          <Input
            type="text" size="large" placeholder="请输入就诊人姓名"
