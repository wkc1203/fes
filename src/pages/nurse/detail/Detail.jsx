...
class NurseDe extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      isOnDuty: true,
  ...
  componentWillUnmount() {
    console.log("his1p1",this.props.location.query.operate);
  ...
  changeDuty = (checked, type) => {
    ...
    if (type == 1) {
      dispatch({
        type: 'nurseDe/changegraphic',
        payload: {isOnDuty: checked ? 1 : 0},
      });
    }
    if (type == 2) {
      dispatch({
        type: 'nurseDe/changevideo',
        payload: {isOnDuty: checked ? 1 : 0},
      });
    }
  }
  ...
  submit = () => {
    const { dispatch } = this.props;
    const {image, name,sex, doctorId, level, mobile,hisDoctorName,  deptId, deptName, specialty, introduction, workingLife } = this.props.detail;
   ...
   /*  if (!circleImage) { message.warning('请上传医生名片', 2); return false; } */
    ...
    if (!sex) { message.warning('请选择性别', 2); return false; }
   ...
   {/*  if(!!this.props.location.query.operate){
    ...
    return (
     ...
     { this.state.showMore&&this.state.img!=''&&<span  className='confirm1' onClick={() => this.uploadImg1()} >上传</span>}
        ...
        <span style={{marginLeft:'24px'}}>性别：</span><Select
        size="large"
        placeholder="请选择性别"
        style={{width:'200px'}}
        value={this.props.detail.sex}
        onChange={value => this.changeDet({sex: value})}
        >
