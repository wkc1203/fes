import React from 'react';
import { connect } from 'dva';
import { hashHistory } from 'dva/router';
import { Table, Input, Select, DatePicker, Button, Icon, Rate,Modal } from 'antd';
import './style.less';
import locale from 'antd/lib/date-picker/locale/zh_CN';

const Option = Select.Option;
const { RangePicker } = DatePicker;

const confirm = Modal.confirm;
const content = (
  <div>数字越大越靠前</div>
);

class Sample extends React.Component {
  state = {
    messageData: [],
    startDate:'',
    endDate:'',
    
  }
  componentDidMount(){
    this.query(2);

  }
  query = (pageNumber) => {
    const { dispatch } = this.props;
    dispatch({
      type: 'message/list',
      payload: {
        hisId:sessionStorage.getItem('hisId'),
        startDate: this.state.startDate?this.state.startDate+ ' 00:00:00':'',
        endDate: this.state.endDate?this.state.endDate+ ' 23:59:59':'',
      },
      callback:(res)=>{
        console.log("res",res)
         if(res.code==0){
           this.setState({
            messageData:res.data
           }) 
          
         }
      }
    });
  }
  onSubmit = () => {
  
    this.query(2);
  }
  onDateChange = (date, dateStr) => {
    this.setState({
      startDate: dateStr[0] || '',
      endDate: dateStr[1] || '',
    });
  }
  submit1 = () => { 
 
      if(this.state.messageData&&this.state.messageData.length>0){
        const form =  document.getElementById('export1');// eslint-disable-line
        form.submit();
      }else{
        message.warning('暂未查询到相关数据!', 2);
      }
    
  }
  
  render() {
   
    return (
      <div className="page-sample-mng">
    
        <div className="query-box" style={{marginBottom:'40px'}}>
          <span>时间：</span>
          <RangePicker size="large"   onChange={this.onDateChange} locale={locale.DatePicker} />
          <Button size="large" type="primary" style={{marginLeft: '20px'}}  onClick={this.onSubmit}>确 定</Button>
          <Button size="large" style={{marginLeft: '50px'}}  onClick={this.submit1}><Icon type="download" />导出 <i className='xia'/>
              
          </Button>
        </div>
        <form
          id="export1" method="post" target="_top"
          action={"/api/mch/user/template/statistics/export?hisId="+sessionStorage.getItem('hisId')+"&startDate="+this.state.startDate+"&endDate="+this.state.endDate}
        />
        {this.state.messageData&&this.state.messageData.length> 0&&
        this.state.messageData.map((item) => {
          return(
            <div className='type'>
                 <div className='left'><p>{item.type}</p></div>
                 <div className='right'>
                    <div className='top'>
                    {
                      item.list&&item.list.length>0&&item.list.map((item1)=>{
                          return(
                            <p>
                                  <p className='name'>{item1.templateName}</p>
                                  <p className='times'>{item1.sendTimes}</p>
                            </p>
                          )
                      })
                    }
                    
                    </div>
                    {item.type=='监管人员'&&<div className='total' style={{'width': '450px'}}>总计：{item.totalSendTimes}</div>}
                    {item.type!=='监管人员'&&<div className='total' >总计：{item.totalSendTimes}</div>}

                 </div>
            </div>
          ) 
        })
       
          }
      
      </div>
    );
  }
}
export default connect((state) => {
  const { sample } = state;
  return {
    ...sample,
  };
})(Sample);
