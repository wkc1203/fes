...
export default {
  ...
  effects: {
    ...
    *actions({ payload }, { call, put }) {
      message.loading('loading...', 0);
      const { data } = yield call(service.actions, payload);
      message.destroy();
      if (data.code === 0) {
        message.success('操作成功！', 2);
        yield put({
    ...
    *ewm({ payload }, { call, put }) {
      // message.loading('loading...', 0);
      const { data } = yield call(service.ewm, payload);
      message.destroy();
      if (data.code === 0) {
    ...
    setup({ dispatch, history }) {
      return history.listen(({ pathname }) => {
        if (pathname === '/drug') {
          ...
          dispatch({type: 'root/save', payload: {breadcrumb: ['药师审方']}});// eslint-disable-line
